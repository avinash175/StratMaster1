...
{
    public class StormStrategy : BasicStrategy
    {
        ...
        public object Reverse = 1;

        public StormStrategy(string stratName, double alloc, double cost, double timeStep)
        ...
        public override void RunStrategy(StrategyData data)
        {
            ...
            int sign = Convert.ToInt32(Reverse);
                ...
                || data.SeriesType == TypeOfSeries.OHLCV)
            {
                for (int i = 0; i < numSec; i++)
                {
                    ...
                    double[] Sig = CalmNess.Select((x, j) => (double)sign * x * BrkOutInd[j]).ToArray();
