...
namespace CommonLib
{
    ...
    public class SpecialFunction
    {
        ...
        /// <param name="a"></param>
        /// <returns></returns>
        public static double normalCDF(double a)
        {
            double x, y, z;
        ...
        /// </summary>
        /// <param name="p"> uniform dist random variable</param>
        /// <returns> N(0,1) random variable</returns>
        public static double inverseCummNormal(double p)
        {
        ...
        /// <summary>
        /// This function returns a normally distributed RV with mean mu and SD sig.
        /// </summary>
        /// <returns></returns>
        public static double GetNormal(double mu, double sig)
        {
            ...
            t1 = rand.NextDouble();
            z = mu + sig * Math.Sqrt(-2 * Math.Log(s1)) * Math.Sin(2 * Math.PI * t1);
            return z;
        }

        public static double[] randN(int numofN, double mu, double sig)
        {
            Random rand = new Random();
            double[] randArray = new double[numofN];
            
            for (int i = 0; i < numofN; i++)
            {
                randArray[i] = mu + sig * inverseCummNormal(rand.NextDouble());
            }

            return randArray;
        }

        public static double[] randU(int numofN, double mu, double sig)
        {
            Random rand = new Random();
            double[] randArray = new double[numofN];
            ...
            for (int i = 0; i < numofN; i++)
            {
                randArray[i] = mu + sig * rand.NextDouble();
            }
        ...
        /// <summary>
        /// This function returns Normal PDF for given mu and variance
        /// </summary>
        /// <param name="x">pdf at x</param>
        /// <param name="mu">mean</param>
        /// <param name="var">variance</param>
        /// <returns>exp(-(x-mu)^2/(2*var))/sqrt(2*pi*var)</returns>
        public static double normalPDF(double x, double mu, double var)
        {
            return Math.Exp(-(x - mu) * (x - mu) / (2 * var)) / Math.Sqrt((2 * Math.PI * var));
        }

        public static double LogOfNormalPDF(double x, double mu, double var)
        {
            return -(x - mu) * (x - mu) / (2 * var) - 0.5 * Math.Log(2 * Math.PI * var);
        }
